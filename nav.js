...
(function () {
    ...
    var NAV_HTML = '<div id="modal-overlay" class="modal-overlay" onclick="window._navBackdrop(event)">'
        ...
        + '<a href="shop.html" class="min-[900px]:hidden flex items-center justify-center hover:text-white transition text-zinc-500" style="width:34px;height:34px;border:1px solid rgba(255,255,255,0.1);border-radius:8px;flex-shrink:0;text-decoration:none"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg></a>'
        + '<a href="exclusive.html" class="min-[900px]:hidden flex items-center justify-center hover:text-white transition text-zinc-500" style="width:36px;height:36px;border:1px solid rgba(255,255,255,0.1);border-radius:8px;flex-shrink:0;text-decoration:none"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 3h12l4 6-10 13L2 9z"/><path d="M11 3L8 9l4 13 4-13-3-6"/><line x1="2" y1="9" x2="22" y2="9"/></svg></a>'
        ...
        + '<a href="shop.html" class="hover:text-white transition whitespace-nowrap" style="font-size:16px;font-weight:700;text-transform:uppercase;letter-spacing:0.2em">Shop</a>'
        + '<span class="text-zinc-800 mx-2">·</span>'
        + '<a href="exclusive.html" data-key="nav-exclusive" class="hover:text-white transition whitespace-nowrap" style="font-size:16px;font-weight:700;text-transform:uppercase;letter-spacing:0.2em">Eksklusiiv</a>'
        + '<span class="text-zinc-800 mx-2">·</span>'
        + '<a href="https://sfflab.ee/#pricing" data-key="nav-pricing" class="hover:text-white transition whitespace-nowrap">Paketid</a>'
        + '</div>'
        + '<div class="min-[900px]:hidden" style="position:relative;flex-shrink:0">'
        + '<button id="lang-dd-btn" onclick="window._toggleLangDD()" style="background:none;border:1px solid rgba(255,255,255,0.12);border-radius:8px;cursor:pointer;color:#71717a;font-size:11px;font-weight:800;letter-spacing:0.15em;padding:6px 9px;display:flex;align-items:center;gap:4px;font-family:inherit;transition:border-color 0.2s,color 0.2s" onmouseover="this.style.color=\'white\';this.style.borderColor=\'rgba(255,255,255,0.3)\'" onmouseout="this.style.color=\'#71717a\';this.style.borderColor=\'rgba(255,255,255,0.12)\'">'
        ...
        + '</div>'
        + '</div>'
        + '<div class="hidden min-[900px]:flex items-center space-x-3 border-l border-white/10 pl-4 text-sm font-bold tracking-widest" style="flex-shrink:0;white-space:nowrap">'
        ...
        + '</div>'
        + '<button id="nav-action-btn" onclick="window._navActionClick ? window._navActionClick() : openModal()" data-key="nav-status" class="nav-status-btn btn-ripple bg-white text-black px-6 py-2.5 rounded-full hover:bg-zinc-200 transition uppercase tracking-widest text-[11px] font-black" style="flex-shrink:0;white-space:nowrap;border:none;cursor:pointer;min-width:11rem;text-align:center">Tellimuse staatus</button>'
        + '</div>'
        + '</div>'
        + '</nav>';
    ...
    function _init() {
        ...
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Escape') { window.closeModal(); window._closeLangDD(); }
        });
    ...
    window.setLanguage = function (lang) {
        ...
        if (_md && tr[lang] && tr[lang]['page-desc']) _md.setAttribute('content', tr[lang]['page-desc']);
        document.querySelectorAll('.lang-btn').forEach(function (b) { b.classList.remove('active'); });
        var ab = document.getElementById('lang-' + lang);
        if (ab) ab.classList.add('active');
        var _ddLbl = document.getElementById('lang-dd-label');
        if (_ddLbl) _ddLbl.textContent = lang.toUpperCase();
